...
create table if not exists public.user_api_keys (
  ...
  provider text not null, -- openai, anthropic, deepseek, gemini, openrouter
  encrypted_key text not null, -- encrypted with service key
...
create table if not exists public.messages (
  ...
  meta jsonb default '{}'::jsonb, -- tokens_in, tokens_out, cost, model, provider, reasoning_content, etc.
  created_at timestamptz default now()
);
...
create table if not exists public.documents (
  ...
  user_id uuid references auth.users on delete cascade not null,
  name text not null,
  file_type text, -- pdf, txt, md, docx, csv, json
  file_size bigint,
  storage_path text, -- path in Supabase Storage
  status text default 'pending' check (status in ('pending', 'processing', 'ready', 'failed')),
  error_message text,
  chunk_count integer default 0,
  metadata jsonb default '{}'::jsonb, -- original filename, page count, etc.
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
...
create table if not exists public.document_chunks (
  ...
  token_count integer,
  metadata jsonb default '{}'::jsonb, -- page_number, section, headers, etc.
  created_at timestamptz default now()
);

-- Index for vector similarity search (IVFFlat - good for < 1M vectors)
...
-- ============================================
-- 9. HYBRID SEARCH FUNCTION (vector + keyword)
-- ============================================
...
-- ============================================
-- DONE! Your Supabase is ready for MULTECH AI
-- ============================================
