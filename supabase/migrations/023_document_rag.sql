-- =============================================================================
-- Migration: Advanced Document RAG with Vector Search
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS document_chunks (
    ...
    -- Vector embedding (1536 for OpenAI, 384 for MiniLM)
...
BEGIN
    ...
    keyword_results AS (
        SELECT 
            dc.id,
            ts_rank_cd(to_tsvector('russian', dc.content), plainto_tsquery('russian', query_text)) AS kw_score
        FROM document_chunks dc
        WHERE 
            (filter_user_id IS NULL OR dc.user_id = filter_user_id)
            AND (filter_document_id IS NULL OR dc.document_id = filter_document_id)
    ...
    SELECT 
        ...
        (vector_weight * v.vec_score + keyword_weight * COALESCE(k.kw_score, 0)) AS combined_score
...
CREATE POLICY "Users can view own documents" ON rag_documents
    FOR SELECT USING (auth.uid()::text = user_id OR user_id = 'dev@example.com');

CREATE POLICY "Users can insert own documents" ON rag_documents
    FOR INSERT WITH CHECK (auth.uid()::text = user_id OR user_id = 'dev@example.com');

CREATE POLICY "Users can delete own documents" ON rag_documents
    FOR DELETE USING (auth.uid()::text = user_id OR user_id = 'dev@example.com');
...
-- Update document timestamp on chunk changes
...
CREATE TRIGGER trigger_update_document_on_chunk
