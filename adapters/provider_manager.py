    ...
    async def load_configurations(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                for provider_id, config in config_data.get("providers", {}).items():
                    try:
    ...
    async def _validate_provider(self, provider_id: ModelProvider, adapter: BaseAdapter):
        ...
        try:
            ...
            if is_valid:
                ...
                self.logger.info(f"Provider {adapter.name}: ✓ Connected, {len(models)} models")
            else:
                self.logger.warning(f"Provider {adapter.name}: ✗ {error}")
                
        except Exception as e:
            status.connected = False
            status.error = str(e)
            self.logger.error(f"Provider {adapter.name}: ✗ {e}")
        finally:
